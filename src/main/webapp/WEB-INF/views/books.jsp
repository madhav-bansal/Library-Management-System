...
<body>
		...
		<div class="collapse navbar-collapse" id="navbarSupportedContent">
			<ul class="navbar-nav mr-auto">
				...
				<c:if test="${admin ne null }">
					<li class="nav-item"><a class="nav-link"
						href="/librarymanagement/librarians">librarians</a><span
	...
	<div class="list-group">
		<h1>
			<label style="color: green; position: relative; left: 50%;">Books
			</label>
		</h1>
		...
		<c:forEach begin="0" end="${qresult.size()}" var="q"
			...
			<div class="card border border-success">
				...
				<div class="card-header   bg-info text-white" name="book name"">${q.name }<c:if
						test="${librarian ne null || admin ne null}">
						<a href="removebook=${q.id}" class="btn btn-danger btn-lg"
							style="position: absolute; right: 0px; top: 0px; padding: 10px;">Remove</a>
					</c:if>
				...
				<div class="card-body">
					<h5 class="card-title">
						Book_ID: <span style="color: #6c757d !important">${q.id }</span>
					</h5>
					<h5 class="card-title">
						Author: <span style="color: #6c757d !important">${q.author }</span>
					</h5>
					<h5 class="card-title">
						Publisher:<span style="color: #6c757d !important">
							${q.publisher }</span>
					</h5>

					<p class="card-text">
						<strong>Description:</strong>${q.desc }</p>
					<h5>
						Copy available: <span style="color: #6c757d !important">${q.availability }</span>
					</h5>
					<c:if test="${librarian ne null || admin ne null }">
						<form action="availbook=${q.id}" method="POST">
							<input type="number" class="h-100 " value="${q.availability }"
								name="availability" style=""> <input type="submit"
								value="update count" name="submit" class="btn btn-info btn-lg">
				...
				<div class="card-footer text-muted">
					...
					<c:if test="${librarian ne null || admin ne null }">
						<c:if test="${q.status == true }">
							<a href="updatebookstatus=${q.id}"> <input type="submit"
					...
					<c:if test="${user ne null }">
						<c:if test="${q.status == true }"> Available  </c:if>
						<c:if test="${q.status==false}"> Currently Not available</c:if>
					</c:if>
	...
	<c:if test="${librarian ne null || admin ne null}">
		<div>
			<a href="addbook" class="btn btn-success btn-lg fixed-bottom	"
				style="">Add New Book</a>
		</div>
	</c:if>
